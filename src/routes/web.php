...
Route::middleware('auth')->group(
    function () {
        ...
        //取引画面でのメッセージ機能
        Route::post('/transactions/{transaction}/messages', [MessageController::class, 'store'])->name('message.store'); //投稿
        Route::delete('/transactions/{transaction}/messages/{message}', [MessageController::class, 'delete'])->name('message.delete'); //削除
        Route::put('/transactions/{transaction}/messages/{message}', [MessageController::class, 'update'])->name('message.update'); //編集
        //取引完了時の評価機能
        Route::post('/transactions/{transaction}/review', [ReviewController::class, 'store'])->name('review.store');
    }
);
